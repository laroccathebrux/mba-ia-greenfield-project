...
import { CreateVideos1782513228444 } from './migrations/1782513228444-CreateVideos';
...
describe('Database migrations (integration)', () => {
  ...
  it('should revert the last migration and remove the videos table', async () => {
    ...
    const videos = await dataSource.query<{ table_name: string }[]>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public' AND table_name = 'videos'`,
    );
    expect(videos).toHaveLength(0);

    // The token tables (earlier migrations) remain intact.
    const tokens = await dataSource.query<{ table_name: string }[]>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public'
         AND table_name = ANY($1::text[])`,
